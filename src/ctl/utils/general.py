...
def denormalize(
    ...
    """
    Invert Albumentations normalization:

        x_norm = (x - mean * M) / (std * M)

    =>

        x = x_norm * (std * M) + mean * M

    Converts BCHW torch tensor -> BHWC numpy float32.

    If mean or std is None: only converts BCHW -> BHWC.
    """
    if x.dim() != 4:
    ...
    if mean is not None and std is not None:
        mean_t = torch.as_tensor(mean, dtype=torch.float32, device="cpu").view(
            1, c, 1, 1
        )
        std_t = torch.as_tensor(std, dtype=torch.float32, device="cpu").view(1, c, 1, 1)

        if mean_t.numel() != c or std_t.numel() != c:
            raise ValueError(f"mean/std must have length C={c}")

        M = float(max_pixel_value)

        x_cpu = x_cpu * (std_t * M) + (mean_t * M)

    # BCHW -> BHWC
    return x_cpu.permute(0, 2, 3, 1).contiguous().numpy().astype(np.float32)
